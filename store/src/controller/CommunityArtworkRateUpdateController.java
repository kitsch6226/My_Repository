...
@Component("/main/updateArtworkRate.go")
public class CommunityArtworkRateUpdateController implements Controller, DataBinding {
	private CommunityDao communityDao;

	public CommunityArtworkRateUpdateController setCommunityDao(CommunityDao communityDao) {
	...
	public String execute(Map<String, Object> model) throws Exception {
		...
		if (loginInfo != null) {
			...
			if(rate!=null&&rate.contentEquals("1")) {
				int result = communityDao.updateArtworkRateGood(loginInfo.getMail(), aseq);
				if(result>=0) {
					return"redirect:community.go";
				}
			}else if(rate!=null&&rate.contentEquals("2")) {
				int result = communityDao.updateArtworkRateBad(loginInfo.getMail(), aseq);
				//db에 잘들어갔다면
				if(result>=0) {
					return"redirect:community.go";
				}
